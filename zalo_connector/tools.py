...
def get_binary_attach(env, model, res_id, field, fields_ret=['mimetype'], product_id=False):
    if product_id:
        size = field.rsplit('_', 1)[1]
        tmpl_id = product_id.product_tmpl_id.id
        attach = env['ir.attachment'].sudo().search_read(
            ['|',
             '&', '&',
             ('res_model', '=', 'product.product'), ('res_id', '=', res_id),
             ('res_field', '=', 'image_variant_' + size),
             '&', '&',
             ('res_model', '=', 'product.template'), ('res_id', '=', tmpl_id), ('res_field', '=', 'image_' + size),
             ], fields=fields_ret,
            limit=1, order='res_model')
    else:
        attach = env['ir.attachment'].sudo().search_read(
...
def phone_format(number, country_id=None, formatted=False, raise_error=True):
    ...
    From Zalo (Is valid number): country_id=None
            ...
            for ttype in list(types):  # for all test case try to find a valid case
                ...
                if good_number.italian_leading_zero or good_number.number_of_leading_zeros:
                    ...
                    if len(bad_number) < len(good_number):  # if original number missing zeros
                        ...
                        new_number = '+%s%s' % \
                                     (numobj.country_code, str(numobj.national_number).rjust(padding_zero, '0'))
